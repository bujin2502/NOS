...
generate_keys_button = Button(asymmetric_frame, text="Kreirati RSA ključeve", command=generate_rsa_keys)
...
generate_aes_key_button = Button(symmetric_frame, text="Kreirati AES ključ", command=generate_aes_key)
...
hash_create_button = Button(hash_frame, text="Kreiraj sažetak", command=lambda: print(save_hash_to_file(hash_file_path.get())))
...
v_signature_create_button = Button(v_sign_frame, text="Ovjeri potpis", command=lambda: verify_signature('javni_kljuc.txt', 'text.txt.sig', v_original_file_path.get()))
